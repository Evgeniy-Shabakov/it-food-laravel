...
class UserAddressStoreRequest extends FormRequest
{
   ...
   public function rules(): array
   {
      return [
         ...
         'latitude' => [
            'nullable',
            'numeric',
            'between:-90,90',  // Широта должна быть между -90 и 90 градусами
            'regex:/^-?\d{1,3}(\.\d{1,18})?$/'  // Формат с точностью до 18 знаков после запятой
         ],
         'longitude' => [
            'nullable',
            'numeric',
            'between:-180,180',  // Долгота должна быть между -180 и 180 градусами
            'regex:/^-?\d{1,3}(\.\d{1,18})?$/'  // Формат с точностью до 18 знаков после запятой
         ],
         'value_string' => ['string', 'max:500', 'nullable'],
      ];
   }
   ...
   public function messages(): array
   {
      return [
         ...
         'house_number.required' => 'Поле "Дом" обязательно для заполнения',
         'house_number.string' => 'Поле "Дом" должно быть строковым значением',
